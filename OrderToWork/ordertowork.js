...
let send_offer_form = document.getElementById('send_offer_form');
...
if (offer_file_revision) {
	// Поле ввода файла
	offer_file_revision.addEventListener("change", function () {
		// Если поле не пустое
		if (this.value) {
			let allowExtension = ['xls', 'xlsx']; // Допустимые разрешения
			let allowSize = 15000000; // Максимальный размер файла
...
if (message_body_revision) {
	// Поле воода сообщение
	message_body_revision.addEventListener("keyup", function () {
		if (this.value.length > 0 && this.value.length <= maxValue) {
			submit_btn.classList.remove('des');
