...
$mail_sending_groups = $_POST['mail_sending_groups'];  // Получатели сообщения (ID)
$mail_sending_subgroup_managers = $_POST['mail_sending_subgroup_managers'];
$mail_sending_subgroup_clients = $_POST['mail_sending_subgroup_clients'];
...
if (isset($_COOKIE['id']) and isset($_COOKIE['hash']))
{
	...
	if(($user_id == intval($_COOKIE['id'])) and ($userdata['user_hash'] == $_COOKIE['hash']) and ($userdata['user_id'] == $_COOKIE['id'])){
		...
		echo "Отправка Менеджеры " . $mail_sending_subgroup_managers ." Клиенты: ".$mail_sending_subgroup_clients;
