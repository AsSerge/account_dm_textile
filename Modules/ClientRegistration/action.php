...
if(isset($_POST['submit']))
{
	...
	if(count($err) == 0)
	{
		...
		// Получаем ID команды менеджера, регистрирующего клиента (руководитель)
		...
		$stmt = $pdo->prepare("INSERT INTO users SET user_login=".$user_login.", user_password='".$user_password."', user_name='".$user_name."', user_surname='".$user_surname."', user_team='".$team_id."', user_role='kln', user_leader='".intval($_COOKIE['id'])."'");
		// $stmt = $pdo->prepare("INSERT INTO users SET user_login=".$user_login.", user_password='".$user_password."', user_name='".$user_name."', user_surname='".$user_surname."', user_role='kln'");
		$stmt->execute();
